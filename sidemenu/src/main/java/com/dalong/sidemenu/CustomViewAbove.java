...
public class CustomViewAbove extends ViewGroup {
	...
	private boolean mIsWidthOffset = true;
			...
			public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
				if (mViewBehind != null) {
					switch (position) {
						case 0:
							mIsWidthOffset =true;
							mViewBehind.setIsWidthOffset(mIsWidthOffset);
							break;
						case 2:
							mIsWidthOffset =false;
							mViewBehind.setIsWidthOffset(mIsWidthOffset);
							break;
						case 1:
							if (positionOffset>0&&positionOffsetPixels>0){
								mIsWidthOffset =false;
								mViewBehind.setIsWidthOffset(mIsWidthOffset);
							}else if (positionOffset<0&&positionOffsetPixels<0){
								mIsWidthOffset =true;
								mViewBehind.setIsWidthOffset(mIsWidthOffset);
							}
							break;
	...
	void setCurrentItemInternal(int item, boolean smoothScroll, boolean always,
		...
		if (mOnRightSlidingMenuListener != null && item == 1 && destX == 0 && mOnRightSlidingMenuListener != null) {
			mOnRightSlidingMenuListener.OnDisplayState(SlidingMenu.SlidingMenuDisplayState.DoNotDisplay);
		} else if (mOnRightSlidingMenuListener != null && item == 2 && destX == getWidth() && mOnRightSlidingMenuListener != null) {
			mOnRightSlidingMenuListener.OnDisplayState(SlidingMenu.SlidingMenuDisplayState.FullyDisplay);
		} else if (mOnLeftSlidingMenuListener != null && item == 0 && destX < 0) {
			mOnLeftSlidingMenuListener.OnDisplayState(SlidingMenu.SlidingMenuDisplayState.PartDisplay);
		}
	}
	...
	private void pageScrolled(int xpos) {
		...
		onPageScrolled(mCurItem, offset, offsetPixels);
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		if(!SlidingFragmentActivity.canScroll)
			return false;
		...
		switch (action & MotionEventCompat.ACTION_MASK) {
			 ...
			 */
				...
				return mQuickReturn;
			case MotionEvent.ACTION_MOVE:
				if (mOnRightSlidingMenuListener != null) {
					mOnRightSlidingMenuListener.OnDisplayState(SlidingMenu.SlidingMenuDisplayState.PartDisplay);
				}
				if (!mIsBeingDragged) {
					determineDrag(ev);
					if (mIsUnableToDrag)
				...
				if (mIsBeingDragged) {
					...
					//LogCat.e(LogCat.TAGAN, "mScrollX:" + mScrollX);
				}
				break;
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (mTransformer != null&&mScrollX <=0 &&mIsWidthOffset) {
			canvas.save();
	...
	private SlidingMenu.OnLeftSlidingMenuListener mOnLeftSlidingMenuListener;

	public void setOnLeftSlidingMenuListener(SlidingMenu.OnLeftSlidingMenuListener leftSlidingMenuListener) {
		mOnLeftSlidingMenuListener = leftSlidingMenuListener;
	}

	private SlidingMenu.OnRightSlidingMenuListener mOnRightSlidingMenuListener;

	public void setOnRightSlidingMenuListener(SlidingMenu.OnRightSlidingMenuListener rightSlidingMenuListener) {
		mOnRightSlidingMenuListener = rightSlidingMenuListener;
	}

	public boolean isBeginDrag(){
